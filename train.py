...
parser.add_argument('--file_storage_path', type=str, default="/volume/", metavar='S',
                    help='file storage path')
parser.add_argument('--pretrained_path', type=str, default="/volume/mask_rcnn_82_AP_96.36.pt", metavar='S',
                    help='ptretrained model path')
parser.add_argument('--num_workers', type=int, default=10, metavar='N',
                    help='num workers')
parser.add_argument('--checkpoint', type=int, default=5000, metavar='N',
                    help='eval period')

parser.add_argument('--wandb_project', type=str, default="kesco_mask", metavar='S',
                    help='wandb project')
parser.add_argument('--wandb_name', type=str, default='quadro_26_classes', metavar='N',
                    help='wandb name')
parser.add_argument('--collect_json', action='store_true', default=False,
                    help='json collect process (default: False)')

parser.add_argument('--train_json', type=str,
                    default="E:\\work\\kesco\\code\\autolabeling+maskrcnn\\output\\train.json", metavar='S',
                    help='train json path')
parser.add_argument('--test_json', type=str, default="E:\\work\\kesco\\code\\autolabeling+maskrcnn\\output\\test.json",
                    metavar='S',
                    help='test json path')
...
def get_config():
    ...
    cfg.SOLVER.STEPS = [10000, 20000, 30000]  # use with WarmupMultiStepLR
...
def train(args):
    ...
    if comm.get_local_rank() == 0:
        ...
        wandb.init(project=args.wandb_project, name=str(args.wandb_name), job_type="Training", config=cfg_wandb,
                   entity="hongmin", group="DDP")
...
if __name__ == '__main__':
    ...
    if len(output_folders) > 0:
        for output_folder in output_folders:
            output_num = int(output_folder.split('_')[-1]) if int(
                output_folder.split('_')[-1]) > output_num else output_num
